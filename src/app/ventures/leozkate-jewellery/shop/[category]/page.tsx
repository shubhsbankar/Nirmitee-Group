...
export default async function Page({
  ...
  if (!category || !CATS.includes(category)) notFound();
  ...
  return (
    <main className="bg-midnight text-white px-6 pb-16">
      <h1 className="text-2xl sm:text-3xl font-semibold mt-6 mb-3 capitalize">
        {category}
      </h1>

      <FiltersBar
        basePath={`/ventures/leozkate-jewellery/shop/${category}`}
      />
...
// Mock data (replace with backend / CMS)
async function fetchProducts({
  ...
  return MOCK.filter(
    (p) => p.category === category || (category === "gold" && p.category === "rings")
  );
}
